...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  V3F body_rates = rot * gyro;

  float predictedPitch = pitchEst + dtIMU * body_rates.y; //predicted pitch 
  float predictedRoll = rollEst + dtIMU * body_rates.x;   //predicted roll
  ekfState(6) = ekfState(6) + dtIMU * body_rates.z;       //predicted yaw
